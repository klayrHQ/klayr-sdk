...
import {
	...
	computeMinFee,
	Options,
} from '@klayr/transactions';
import { address as cryptoAddress, ed } from '@klayr/cryptography';
...
export class TransactionMethods {
	...
	public async create<T = Record<string, unknown>>(
		...
		const address = cryptoAddress.getKlayr32AddressFromPublicKey(publicKey);
	...
	public async sign(
		...
		const chainID = Buffer.from(this._nodeInfo.chainID, 'hex');
		const address = cryptoAddress.getKlayr32AddressFromPublicKey(decodedTx.senderPublicKey);
