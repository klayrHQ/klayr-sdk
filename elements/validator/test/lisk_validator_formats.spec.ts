...
describe('validator formats', () => {
	...
	describe('klayr32', () => {
		...
		it('should validate to true when valid hex string is provided', () => {
			expect(() =>
				validator.validate(schema, { target: 'klyycz7hvr8yfu74bcwxy2n4mopfmjancgdvxq8xz' }),
			).not.toThrow();
		});

		it('should validate to false when address is in hex', () => {
			const expectedError =
				'Klayr validator found 1 error[s]:\nProperty \'.target\' must match format "klayr32"';
		...
		it('should validate to false when address is invalid', () => {
			...
			expect(() =>
				validator.validate(schema, {
					target: 'klyycz7hvr8yfu74bcwxy2n4mopfmjancgdvxqzzz',
				}),
			).toThrow(expectedError);
		});
	});
	...
	describe('path', () => {
		...
		it('should validate to false for invalid path', () => {
			expect(() => validator.validate(pathSchema, { rootPath: 'klayr' })).toThrow(
				'Property \'.rootPath\' must match format "path"',
			);
		});

		it('should validate to true for valid path with tilde', () => {
			expect(() => validator.validate(pathSchema, { rootPath: '~/.klayr' })).not.toThrow();
		});

		it('should validate to true for valid path', () => {
			expect(() => validator.validate(pathSchema, { rootPath: '/tmp/klayr/test/' })).not.toThrow();
		});
	});
