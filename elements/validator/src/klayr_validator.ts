...
import { convertErrorsToLegacyFormat, KlayrvalidationError } from './errors';
import { fieldNumberKeyword } from './keywords/field_number';
import { dataTypeKeyword } from './keywords/data_type';
import { klayrmetaSchema } from './klayr_meta_schema';
import { KlayrerrorObject } from './types';

export const klayrschemaIdentifier: string = klayrmetaSchema.$id;

export class KlayrValidator {
	...
	public constructor() {
		this._validator = new Ajv({
			...
			strictSchema: true,
			allErrors: true,
			// FIXME: Combination with klayr-codec schema, making true would throw error because
		...
		this._validator.compile(klayrmetaSchema);

		this._validator.addMetaSchema(klayrmetaSchema);
	...
	public validate<T = Record<string, unknown>>(schema: object, data: unknown): asserts data is T {
		if (!this._validator.validate(schema, data)) {
			throw new KlayrvalidationError(
				convertErrorsToLegacyFormat(this._validator.errors as KlayrerrorObject[]),
			);
		}
	}
	...
	public compile(schema: object | boolean): ValidateFunction {
		try {
			...
			if (error instanceof KlayrvalidationError) {
				throw error;
			}
...
export const validator = new KlayrValidator();
