...
const errorFormatter = (error: KlayrerrorObject): string =>
	(errorFormatterMap[error.keyword] ?? defaultErrorFormatter)(error);

export class KlayrvalidationError extends Error {
	public readonly errors: KlayrerrorObject[];

	public constructor(errors: KlayrerrorObject[]) {
		super();
		this.errors = convertErrorsToLegacyFormat(errors);

		this.message = `Klayr validator found ${
			this.errors.length
