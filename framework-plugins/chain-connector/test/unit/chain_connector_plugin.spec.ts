...
import { Types, apiClient, cryptography, db, testing, Modules } from 'klayr-sdk';
import { ChainConnectorPlugin } from '../../src';
...
describe('ChainConnectorPlugin', () => {
	...
	const appConfigForPlugin: Types.ApplicationConfigForPlugin = {
		...
		system: {
			dataPath: '~/.klayr/ChainConnectorPlugin/test/',
		} as any,
	};
	...
	beforeEach(async () => {
		...
		defaultConfig = {
			receivingChainIPCPath: '~/.klayr/mainchain',
			...
			receivingChainID: Modules.Interoperability.getMainchainID(ownChainID).toString('hex'),
		};
			...
			await expect(
				chainConnectorPlugin.init({
					...
					config: {
						...
						...defaultConfig,
						maxCCUSize: (CCU_TOTAL_CCM_SIZE as number) + 1000,
					},
					logger,
				}),
