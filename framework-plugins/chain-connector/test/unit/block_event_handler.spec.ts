...
import { codec, cryptography, Modules, testing } from 'klayr-sdk';
...
describe('BlockEventHandler', () => {
	...
	beforeEach(() => {
		blockEventHandlerConfig = {
			...
			receivingChainID: receivingChainIDBuf,
			maxCCUSize: Modules.Interoperability.MAX_CCM_SIZE,
			ccuFee: '100000',
			isSaveCCU: false,
			ccuSaveLimit: 300,
		};
	...
	describe('_handleNewBlock', () => {
		...
		const sidechainLastCertificate: Modules.Interoperability.LastCertificate = {
			height: 1,
			stateRoot: cryptography.utils.hash(Buffer.alloc(2)),
			timestamp: Math.floor(Date.now() / 1000),
			validatorsHash: cryptography.utils.hash(Buffer.alloc(2)),
		};
		const sidechainChainAccount: Modules.Interoperability.ChainAccount = {
		...
		beforeEach(async () => {
			...
			sampleBlockHeader.generatorAddress = 'klyoaknq582o6fw7sp82bm2hnj7pzp47mpmbmux2g';
			jest.spyOn(blockEventHandler as any, '_initializeReceivingChainClient');
			await blockEventHandler.load(initArgs);
		});
		...
		beforeEach(async () => {
			...
			blockEventHandler['_interoperabilityMetadata'] = {
				stores: [
					{
						key: '1',
						data: Modules.Interoperability.ccmSchema,
					},
				],
				events: [
					{
