...
import { db, testing, cryptography, chain, codec, Modules, Engine } from 'klayr-sdk';
...
describe('DB', () => {
	...
	const dbName = 'klayr-framework-chain-connector-plugin.db';
		...
		describe('validatorsData', () => {
			...
			describe('get/setValidatorsDataByHash', () => {
				...
				it('should return undefined for hash with no validators data', async () => {
					await expect(
						chainConnectorDB.getValidatorsDataByHash(
							cryptography.utils.hash(Modules.Interoperability.EMPTY_BYTES),
						),
					).resolves.toBeUndefined();
				});
			});
		...
		describe('CCUs', () => {
			...
			beforeEach(() => {
				...
				const params = {
					...
					certificate: codec.encode(Engine.certificateSchema, certificate),
