...
export class ChainConnectorPlugin extends Plugins.BasePlugin<ChainConnectorPluginConfig> {
	...
	private _chainConnectorPluginDB!: klayrdB.Database;
	...
	public async _getCcuFee(tx: Record<string, unknown>): Promise<bigint> {
		...
		const userBalance = await this._receivingChainClient.invoke<{ exists: boolean }>(
			'token_hasUserAccount',
			{
				...
				tokenID: `${getTokenIDKLY(this._receivingChainID).toString('hex')}`,
			},
		);
	...
	/**
	 * @see https://github.com/Klayrhq/lips/blob/main/proposals/lip-0053.md
	...
	/**
	 * @see https://github.com/Klayrhq/lips/blob/main/proposals/lip-0053.md#cross-chain-update-transaction-properties
