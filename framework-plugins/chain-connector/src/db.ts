...
import { codec, db as klayrDB, chain, cryptography, Modules, Engine } from 'klayr-sdk';
...
export class ChainConnectorDB {
	...
	public async saveToDBOnNewBlock(blockHeader: BlockHeader) {
		...
		if (
			!blockHeader.aggregateCommit.aggregationBits.equals(Modules.Interoperability.EMPTY_BYTES) ||
			!blockHeader.aggregateCommit.certificateSignature.equals(Modules.Interoperability.EMPTY_BYTES)
		) {
			const aggregateCommitHeight = uint32BE(blockHeader.aggregateCommit.height);
			const aggregateCommitBytes = codec.encode(
				Engine.aggregateCommitSchema,
				blockHeader.aggregateCommit,
			);
			batch.set(
	...
	public async getAggregateCommitByHeight(height: number) {
		try {
			...
			return codec.decode<Engine.AggregateCommit>(Engine.aggregateCommitSchema, bytes);
		} catch (error) {
			checkDBError(error);
	...
	public async getAggregateCommitBetweenHeights(fromHeight: number, toHeight: number) {
		...
		return aggregateCommits.map(a =>
			codec.decode<Engine.AggregateCommit>(Engine.aggregateCommitSchema, a),
		);
	}
	...
	public async getCCMsByHeight(height: number): Promise<Modules.Interoperability.CCMsg[]> {
		const heightBuf = uint32BE(height);
		let crossChainMessages: Modules.Interoperability.CCMsg[] = [];
		try {
			const encodedInfo = await this._db.get(concatDBKeys(DB_KEY_CROSS_CHAIN_MESSAGES, heightBuf));
			crossChainMessages = codec.decode<{ ccms: Modules.Interoperability.CCMsg[] }>(
				ccmsAtHeightSchema,
				encodedInfo,
			).ccms;
		} catch (error) {
		...
		return {
			list: ccuBytes
				.map(b => {
					...
					const params = codec.decode<CCUpdateParams>(
						Modules.Interoperability.ccuParamsSchema,
						tx.params,
					);
					const certificate = codec.decode<Engine.Certificate>(
						Engine.certificateSchema,
						params.certificate,
					);

					const inboxUpdate = {
						crossChainMessages: params.inboxUpdate.crossChainMessages.map(ccmBytes => {
							const ccm = codec.decode<Modules.Interoperability.CCMsg>(
