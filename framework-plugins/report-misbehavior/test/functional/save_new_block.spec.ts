...
import { testing, Types, chain, db as klayrdB, codec } from 'klayr-sdk';
...
describe('save block header', () => {
	...
	const rootPath = join(homedir(), '.klayr', 'report-misbehavior-plugin');
	const apiPort = 5002;
	...
	beforeAll(async () => {
		...
		dbKey = Buffer.concat([
			...
			Buffer.from(':', 'utf8'),
			klayrdB.formatInt(blockHeader.height),
		]);
	});
	...
	describe('from same generator', () => {
		...
		it('should save block headers with different height', async () => {
			...
			dbKey = Buffer.concat([
				...
				Buffer.from(':', 'utf8'),
				klayrdB.formatInt(updatedBlockHeader.height),
			]);
		...
		it('should save different block header with same height', async () => {
			...
			dbKey = Buffer.concat([
				...
				Buffer.from(':', 'utf8'),
				klayrdB.formatInt(modifiedBlockHeader.height),
			]);
			const newBlockHeader = encodeBlockHeader(
				pluginInstance['apiClient'].schemas,
				modifiedBlockHeader,
