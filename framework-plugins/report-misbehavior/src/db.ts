...
import { cryptography, codec, chain, db as klayrdB, Engine } from 'klayr-sdk';

const { BlockHeader } = chain;
const { utils } = cryptography;

export const blockHeadersSchema = {
	$id: '/klayr/reportMisbehavior/blockHeaders',
	type: 'object',
...
export const getDBInstance = async (
	dataPath: string,
	dbName = 'klayr-framework-report-misbehavior-plugin.db',
): Promise<klayrdB.Database> => {
	const dirPath = join(dataPath.replace('~', os.homedir()), 'plugins/data', dbName);
	await ensureDir(dirPath);

	return new klayrdB.Database(dirPath);
};
