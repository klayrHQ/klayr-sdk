 ...
 */
import { Plugins, db as klayrdB, codec, chain, cryptography, blockHeaderSchema } from 'klayr-sdk';
import {
	getDBInstance,
	saveBlockHeaders,
...
export class ReportMisbehaviorPlugin extends Plugins.BasePlugin<ReportMisbehaviorPluginConfig> {
	...
	public async load(): Promise<void> {
		// TODO: https://github.com/Klayrhq/klayr-sdk/issues/6201
	...
	private async _createPoMTransaction(
		...
		const authAccount = await this.apiClient.invoke<{ nonce: string }>('auth_getAuthAccount', {
			address: address.getKlayr32AddressFromPublicKey(this._state.publicKey),
		});
