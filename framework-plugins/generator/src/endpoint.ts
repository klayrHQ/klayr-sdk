...
export class Endpoint extends Plugins.BasePluginEndpoint {
	...
	private _db!: klayrdB.Database;

	public init(db: klayrdB.Database, apiClient: Plugins.BasePlugin['apiClient']) {
		this._db = db;
		...
		for (const account of forgerAccounts) {
			...
			result.push({
				...
				stakers: forgerInfo.stakeReceived.map(stake => ({
					address: cryptography.address.getKlayr32AddressFromAddress(stake.address),
